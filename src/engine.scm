          ...
          (list (new-corout
                 ...
                 (lambda ()
                  ...
                  ;; Setup level and start primordial game corouts
                   (new-player! level)
                   (spawn-brother-thunk 'invader-laser-corout
                                        (lambda ()
                                          (sleep-for 1)
                                          ((create-invader-laser level))))
                   (spawn-brother-thunk 'mother-ship-corout
                                        (lambda ()
                                          (sleep-for (mothership-random-delay))
      ...
      (if (null? invaders)
          ...
          (let* (;; sample an arbitrary invader's speed
            ...
            (if (game-level-wall-collision-detected? level)
                (begin
                  (create-invader-row-move! 0 (- invader-y-movement-speed)
                                            level)
                  (sleep-for dt)
                  (create-invader-wall-movement-continuation old-dx level)
...
(define (create-new-mothership level)
  (synchronized-thunk level
    (let* ((dx-mult (list-ref '(1 -1) (random-integer 2)))
      ...
      (begin (yield) (create-mothership level)))))
...
(define (explode-laser! level laser-obj)
  ...
  (let* ((expl-type-constructor (if (player_laser? laser-obj)
    ...
    (sleep-for animation-duration)
    (create-explosion-end! level obj)))
...
(define (create-animate-score-adv-table level)
  ...
  (lambda ()
    ...
    (set! other-animations-index (modulo (+ other-animations-index 1)
        ...
        (cond
         ...
         ((eq? result 'demo)
          (let ((ai (new-corout 
            ...
            (inf-loop hi-score (boot (list ai)))))
         ...
         ((eq? result 'start-1p-game)
          (let ((p1 (new-corout 
            ...
            (inf-loop hi-score (boot (list p1)))))
         ...
         ((eq? result 'start-2p-game)
          (parameterize ((p1-level (new-level 0 hi-score 2 player1))
           ...
           (let ((p1
             ...
             (boot (list p1 p2))
             (inf-loop hi-score (maxiumum (map corout-get-result p1 p2))))))
